...
public class PostgresSplitOptimizer {
    ...
    public List<PostgresCopySourceSplit> generateOptimizedSplits(
        ...
        log.info(
                "Generating {} splits for table {}, total records: {}",
                numSplits,
                table,
                totalRecords);
        ...
        if (numSplits > 1 && totalRecords > params.recordsPerSplit) {
            splits =
                    generateSplitsWithPrecomputedBoundaries(
                            connection,
                            table,
                            splitBy,
                            where,
    ...
    /** 构建边界查询SQL */
    private String buildBoundariesSql(
            String table, String splitBy, String baseWhereClause, SplitCalculationParams params) {

        String whereCondition = baseWhereClause.isEmpty() ? "" : baseWhereClause + " AND ";
    ...
    private String buildCopySql(String cols, String table, String rangeWhere) {
        return String.format(
                "COPY (SELECT %s FROM %s WHERE %s) TO STDOUT", cols, table, rangeWhere);
    }

    /** 生成单个分片 */
    private List<PostgresCopySourceSplit> generateSingleSplit(
            String table, String where, String cols) {
        List<PostgresCopySourceSplit> splits = new ArrayList<>();
