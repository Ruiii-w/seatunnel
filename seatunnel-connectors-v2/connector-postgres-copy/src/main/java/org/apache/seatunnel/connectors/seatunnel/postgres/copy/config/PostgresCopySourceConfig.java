...
public class PostgresCopySourceConfig implements Serializable {
    ...
    public PostgresCopySourceConfig(ReadonlyConfig config) {
        ...
        // 采样分片配置，使用默认值
        this.splitEvenDistributionFactorUpperBound =
                config.getOptional(PostgresCopyOptions.SPLIT_EVEN_DISTRIBUTION_FACTOR_UPPER_BOUND)
                        .orElse(100.0);
        this.splitEvenDistributionFactorLowerBound =
                config.getOptional(PostgresCopyOptions.SPLIT_EVEN_DISTRIBUTION_FACTOR_LOWER_BOUND)
                        .orElse(0.05);
        this.splitSampleShardingThreshold =
                config.getOptional(PostgresCopyOptions.SPLIT_SAMPLE_SHARDING_THRESHOLD)
                        .orElse(1000);
        this.splitInverseSamplingRate =
                config.getOptional(PostgresCopyOptions.SPLIT_INVERSE_SAMPLING_RATE).orElse(1000.0);
    }
}
