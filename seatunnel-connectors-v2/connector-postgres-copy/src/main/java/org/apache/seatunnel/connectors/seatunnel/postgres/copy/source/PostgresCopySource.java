...
public class PostgresCopySource
    ...
    private CatalogTable buildCatalogTableFromConfig(
        ...
        // 5. 构建注释
        String comment =
                config.getOptional(Options.key("comment").stringType().noDefaultValue())
    ...
    private TableIdentifier buildTableIdentifier(ReadonlyConfig config) {
        String catalogName =
                config.getOptional(
                                Options.key("catalog")
                                        .stringType()
                                        .defaultValue("postgres_copy_catalog"))
                        .orElse("postgres_copy_catalog");

        String databaseName =
                config.getOptional(Options.key("database").stringType().defaultValue("public"))
                        .orElse("public");
        ...
        String tableName =
                config.getOptional(
                                Options.key("table")
                                        .stringType()
                                        .defaultValue("postgres_copy_table"))
    ...
    private Map<String, String> buildTableOptions(ReadonlyConfig config) {
        ...
        // 使用正确的 Options 方式
        config.getOptional(Options.key("copy-format").stringType().defaultValue("csv"))
                .ifPresent(format -> options.put("copy.format", format));

        config.getOptional(Options.key("delimiter").stringType().defaultValue(","))
                .ifPresent(delim -> options.put("copy.delimiter", delim));

        config.getOptional(Options.key("null-string").stringType().defaultValue("\\N"))
