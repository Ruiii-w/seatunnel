...
public class DynamicChunkSplitter extends ChunkSplitter {
    ...
    // 将 Java 值转为 SQL 字面值（Postgres 兼容）
    private String toSqlLiteral(Object value, SeaTunnelDataType<?> type) {
        ...
        switch (type.getSqlType()) {
            ...
            case TIME:
            case TIMESTAMP:
                // 直接以字符串形式比较，Postgres 会按列类型隐式转换
                return "'" + escapeSqlString(String.valueOf(value)) + "'";
            case BYTES:
                throw new UnsupportedOperationException("COPY 分片键暂不支持 BYTES 类型字面值");
            default:
                throw new UnsupportedOperationException("不支持的分片键类型: " + type.getSqlType());
        }
    }
